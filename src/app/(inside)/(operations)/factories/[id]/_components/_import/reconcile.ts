...
export const suggestMatch = (value: string, options: string[]): string | null => {
 ...
 */
export const useReconciliation = (values: string[], existingLabels: string[]) => {
  ...
  useEffect(() => {
    setRecon((prev) => {
      ...
      for (const value of values) {
        next[value] = prev[value] ?? suggestMatch(value, existingLabels) ?? value;
      }
      return next;
    });
