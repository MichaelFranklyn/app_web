...
export function EditVisitModal({ item, open, onOpenChange, onDone }: Props) {
  const formRef = useRef<FormBuilderRef>(null);
  const [updateItem] = useMutation(UPDATE_VISIT_ITEM_MUTATION);
  ...
  const handleSubmit = async (data: Record<string, unknown>) => {
    ...
    const status = extractValue(data.status);
    if (status) input.status = status;
    const outcome = extractValue(data.outcome);
