 ...
 *
 * Só para gestão: na visão do vendedor, `amount` já É a fatia dele e a
...
export function OfficeSplitPanel({ rows, month, sellerName }: Props) {
  ...
  // Fatia nula = o vínculo do vendedor com a fábrica repassa 100% da comissão,
  // que é o padrão de quem nunca definiu a divisão. Sem dizer isso, a linha
  // parece anunciar um mês em que o escritório não ganhou nada.
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-8 gap-y-4 rounded-(--r-md) bg-(--bg3) px-16 py-12">
      ...
      <Title variant="body-sm" color="muted">
        {semDivisao
          ? "no escritório (repasse de 100% ao vendedor — a fatia é definida no vínculo dele com a fábrica)"
